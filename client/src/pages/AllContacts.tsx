...
function initEditState(c: any): EditState {
  ...
  return {
    ...
    regionId: c.regionId?.toString() ?? NONE,
    vendorSubcategoryId: c.vendorSubcategoryId?.toString() ?? NONE,
    clientSubcategoryId: c.clientSubcategoryId?.toString() ?? NONE,
    consultantSubcategoryId: c.consultantSubcategoryId?.toString() ?? NONE,
    contactSourceId: c.contactSourceId?.toString() ?? NONE,
    notes: c.notes ?? "",
  };
}
...
export default function AllContacts() {
  ...
  const [bulkEditData, setBulkEditData] = useState<{
    regionId?: string; contactSourceId?: string;
    vendorSubcategoryId?: string; clientSubcategoryId?: string; consultantSubcategoryId?: string;
  }>({});
  ...
  const { data, isLoading } = trpc.contacts.list.useQuery({
    ...
    ...filters,
    departmentFilter: isAdmin && departmentFilter !== NONE ? departmentFilter : undefined,
    page,
    pageSize: 20,
  ...
  const activeFilterCount = Object.values(filters).filter(Boolean).length + (departmentFilter !== NONE ? 1 : 0);

  const clearFilters = () => { setFilters({}); setDepartmentFilter(NONE); setPage(1); };
  ...
  const hasBulkEdits = Object.values(bulkEditData).some(v => v && v !== NONE);

  const handleBulkEdit = () => {
    const ids = Array.from(selectedIds);
    const payload: any = { ids };
    if (bulkEditData.regionId && bulkEditData.regionId !== NONE) payload.regionId = Number(bulkEditData.regionId);
    if (bulkEditData.contactSourceId && bulkEditData.contactSourceId !== NONE) payload.contactSourceId = Number(bulkEditData.contactSourceId);
    if (bulkEditData.vendorSubcategoryId && bulkEditData.vendorSubcategoryId !== NONE) payload.vendorSubcategoryId = Number(bulkEditData.vendorSubcategoryId);
    if (bulkEditData.clientSubcategoryId && bulkEditData.clientSubcategoryId !== NONE) payload.clientSubcategoryId = Number(bulkEditData.clientSubcategoryId);
    if (bulkEditData.consultantSubcategoryId && bulkEditData.consultantSubcategoryId !== NONE) payload.consultantSubcategoryId = Number(bulkEditData.consultantSubcategoryId);
  ...
  const handleSaveEdit = () => {
    ...
    updateMutation.mutate({
      ...
      phoneNumbers: phones,
      emails: emails,
      regionId: editContact.regionId !== NONE ? Number(editContact.regionId) : null,
      vendorSubcategoryId: editContact.vendorSubcategoryId !== NONE ? Number(editContact.vendorSubcategoryId) : null,
      clientSubcategoryId: editContact.clientSubcategoryId !== NONE ? Number(editContact.clientSubcategoryId) : null,
      consultantSubcategoryId: editContact.consultantSubcategoryId !== NONE ? Number(editContact.consultantSubcategoryId) : null,
      contactSourceId: editContact.contactSourceId !== NONE ? Number(editContact.contactSourceId) : null,
    ...
    <div className="space-y-5">
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <label className="flex items-center gap-2 cursor-pointer select-none shrink-0">
              <Checkbox checked={phoneticSearch} onCheckedChange={(v) => setPhoneticSearch(!!v)} id="phonetic" />
              <span className="flex items-center gap-1 text-xs text-muted-foreground">
                <AudioWaveform className="h-3 w-3" /> Phonetic
              </span>
            </label>
          ...
          {showFilters && (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 mt-3 pt-3 border-t">
              <Select value={filters.regionId?.toString() ?? NONE} onValueChange={v => setFilter("regionId", v !== NONE ? Number(v) : undefined)}>
              ...
              <Select value={filters.vendorSubcategoryId?.toString() ?? NONE} onValueChange={v => setFilter("vendorSubcategoryId", v !== NONE ? Number(v) : undefined)}>
              ...
              <Select value={filters.clientSubcategoryId?.toString() ?? NONE} onValueChange={v => setFilter("clientSubcategoryId", v !== NONE ? Number(v) : undefined)}>
              ...
              <Select value={filters.consultantSubcategoryId?.toString() ?? NONE} onValueChange={v => setFilter("consultantSubcategoryId", v !== NONE ? Number(v) : undefined)}>
                <SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Consultant" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value={NONE}>All Consultants</SelectItem>
                  {metadata.consultantSubcategories.map(s => <SelectItem key={s.id} value={s.id.toString()}>{s.name}</SelectItem>)}
                </SelectContent>
              </Select>
              <Select value={filters.contactSourceId?.toString() ?? NONE} onValueChange={v => setFilter("contactSourceId", v !== NONE ? Number(v) : undefined)}>
              ...
              <TableHeader>
                <TableRow className="hover:bg-transparent">
                  <TableHead className="w-10 px-3">
                    <Checkbox checked={contacts.length > 0 && selectedIds.size === contacts.length} onCheckedChange={toggleAll} />
                  </TableHead>
                ...
                {!isLoading && contacts.map((c: any) => {
                  ...
                  return (
                    <TableRow key={c.id} className={`hover:bg-muted/30 ${selectedIds.has(c.id) ? "bg-indigo-500/5" : ""}`}>
                      <TableCell className="px-3"><Checkbox checked={selectedIds.has(c.id)} onCheckedChange={() => toggleRow(c.id)} /></TableCell>
                      ...
                      <TableCell className="py-3">
                        <div className="flex items-center gap-1">
                          {canEdit(c) && (
                            <Button size="icon" variant="ghost" className="h-7 w-7 text-muted-foreground hover:text-foreground" onClick={() => setEditContact(initEditState(c))}>
                              <Pencil className="w-3.5 h-3.5" />
                            </Button>
                          )}
                          {(isAdmin || c.uploadedByUserId === user?.id) && (
                            <Button size="icon" variant="ghost" className="h-7 w-7 text-muted-foreground hover:text-red-400" onClick={() => setDeleteContactId(c.id)}>
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t">
            ...
            {totalPages > 1 && (
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-1">
                  <Button variant="outline" size="icon" className="h-7 w-7" onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}><ChevronLeft className="w-3.5 h-3.5" /></Button>
                  <Button variant="outline" size="icon" className="h-7 w-7" onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages}><ChevronRight className="w-3.5 h-3.5" /></Button>
                </div>
              </div>
            )}
          </div>
          ...
          {editContact && (
            <div className="space-y-4 mt-1">
              <div>
                ...
                <Input value={editContact.displayName} onChange={e => setEditContact(s => s ? { ...s, displayName: e.target.value } : s)} placeholder="Full name or company name" className="h-9 text-sm" />
              </div>
              <div>
                ...
                <div className="space-y-2">
                  {editContact.phones.map((ph, i) => (
                    <div key={i} className="flex gap-2">
                      <Input value={ph} onChange={e => setEditContact(s => { if (!s) return s; const phones = [...s.phones]; phones[i] = e.target.value; return { ...s, phones }; })} placeholder="+91 98765 43210" className="h-9 text-sm flex-1" />
                      {editContact.phones.length > 1 && (
                        <Button size="icon" variant="ghost" className="h-9 w-9 shrink-0 text-muted-foreground hover:text-red-400" onClick={() => setEditContact(s => s ? { ...s, phones: s.phones.filter((_, idx) => idx !== i) } : s)}>
                          <X className="w-3.5 h-3.5" />
                        </Button>
                      )}
                    </div>
                  ))}
                  <Button size="sm" variant="ghost" className="gap-1.5 text-xs text-muted-foreground h-8" onClick={() => setEditContact(s => s ? { ...s, phones: [...s.phones, ""] } : s)}>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {editContact.emails.map((em, i) => (
                    <div key={i} className="flex gap-2">
                      <Input value={em} onChange={e => setEditContact(s => { if (!s) return s; const emails = [...s.emails]; emails[i] = e.target.value; return { ...s, emails }; })} placeholder="email@example.com" className="h-9 text-sm flex-1" type="email" />
                      {editContact.emails.length > 1 && (
                        <Button size="icon" variant="ghost" className="h-9 w-9 shrink-0 text-muted-foreground hover:text-red-400" onClick={() => setEditContact(s => s ? { ...s, emails: s.emails.filter((_, idx) => idx !== i) } : s)}>
                          <X className="w-3.5 h-3.5" />
                        </Button>
                      )}
                    </div>
                  ))}
                  <Button size="sm" variant="ghost" className="gap-1.5 text-xs text-muted-foreground h-8" onClick={() => setEditContact(s => s ? { ...s, emails: [...s.emails, ""] } : s)}>
              ...
              <div>
                ...
                <Textarea value={editContact.notes} onChange={e => setEditContact(s => s ? { ...s, notes: e.target.value } : s)} placeholder="Any additional notes..." className="text-sm resize-none" rows={3} />
              </div>
            </div>
          )}
          <DialogFooter className="mt-4">
            <Button variant="outline" onClick={() => setEditContact(null)}>Cancel</Button>
            <Button onClick={handleSaveEdit} disabled={updateMutation.isPending || !editContact?.displayName.trim()} className="bg-indigo-600 hover:bg-indigo-700">
      ...
      <AlertDialog open={deleteContactId !== null} onOpenChange={(open) => !open && setDeleteContactId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This action cannot be undone. The contact will be permanently deleted.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={() => deleteContactId !== null && deleteMutation.mutate({ id: deleteContactId })} className="bg-red-600 hover:bg-red-700">
      ...
      <Dialog open={showBulkEdit} onOpenChange={setShowBulkEdit}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>Bulk Edit {selectedIds.size} Contact{selectedIds.size > 1 ? "s" : ""}</DialogTitle></DialogHeader>
          <p className="text-sm text-muted-foreground">Only filled fields will be updated. Leave blank to keep existing values.</p>
          <div className="space-y-3 mt-2">
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1 block">Region</label>
              <Select value={bulkEditData.regionId ?? NONE} onValueChange={v => setBulkEditData(d => ({ ...d, regionId: v }))}>
            ...
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1 block">Source</label>
              <Select value={bulkEditData.contactSourceId ?? NONE} onValueChange={v => setBulkEditData(d => ({ ...d, contactSourceId: v }))}>
                <SelectTrigger className="h-9 text-sm"><SelectValue placeholder="Keep existing" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value={NONE}>Keep existing</SelectItem>
                  {metadata.contactSources.map(s => <SelectItem key={s.id} value={s.id.toString()}>{s.name}</SelectItem>)}
            ...
            <div>
              ...
              <Select value={bulkEditData.vendorSubcategoryId ?? NONE} onValueChange={v => setBulkEditData(d => ({ ...d, vendorSubcategoryId: v }))}>
                <SelectTrigger className="h-9 text-sm"><SelectValue placeholder="Keep existing" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value={NONE}>Keep existing</SelectItem>
                  {metadata.vendorSubcategories.map(s => <SelectItem key={s.id} value={s.id.toString()}>{s.name}</SelectItem>)}
            ...
            <div>
              ...
              <Select value={bulkEditData.clientSubcategoryId ?? NONE} onValueChange={v => setBulkEditData(d => ({ ...d, clientSubcategoryId: v }))}>
                <SelectTrigger className="h-9 text-sm"><SelectValue placeholder="Keep existing" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value={NONE}>Keep existing</SelectItem>
                  {metadata.clientSubcategories.map(s => <SelectItem key={s.id} value={s.id.toString()}>{s.name}</SelectItem>)}
            ...
            <div>
              ...
              <Select value={bulkEditData.consultantSubcategoryId ?? NONE} onValueChange={v => setBulkEditData(d => ({ ...d, consultantSubcategoryId: v }))}>
                <SelectTrigger className="h-9 text-sm"><SelectValue placeholder="Keep existing" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value={NONE}>Keep existing</SelectItem>
                  {metadata.consultantSubcategories.map(s => <SelectItem key={s.id} value={s.id.toString()}>{s.name}</SelectItem>)}
          ...
          <DialogFooter className="mt-4">
            ...
            <Button onClick={handleBulkEdit} disabled={bulkUpdateMutation.isPending || !hasBulkEdits} className="bg-indigo-600 hover:bg-indigo-700">
      ...
      <AlertDialog open={showBulkDelete} onOpenChange={setShowBulkDelete}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This action cannot be undone. All selected contacts will be permanently deleted.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={() => bulkDeleteMutation.mutate({ ids: Array.from(selectedIds) })} className="bg-red-600 hover:bg-red-700">
